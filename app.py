...
# GET com Id - GET http://localhost:5000/autores/1
...
# Criar uma nova postagem - POST http://localhost:5000/autores/
...
def alterar_autor(autor,id_autor):
    ...
    autor = Autor.query.filter_by(id_autor=id_autor).first()
    if not autor:
        return jsonify({'Mensagem':'Esse usuário não for encontrado'})
    try:
...
# Alterar uma postagem existente - DELETE http://localhost:5000/autores/1
...
def excluir_autor(autor,id_autor):
    ...
    return jsonify({'mensagem':'Autor excuído com sucesso!'})
